...
public class UgurBeySession {
    public static void main(String[] args) {
        ...
        method8ByteArray(str);// Çok önemli for döngüsünü incele
    ...
    private static void method8ByteArray(String str) {
        ...
        for (int l = 0, r = str.length() - 1; l < r; l++, r--) {// iki tane l ve r int tanımladık bir temp le yer değiştirdik
            byte temp = bytes[l];
            bytes[l] = bytes[r];
            bytes[r] = temp;
        }
    ...
    private static void method11Stack(String str) {
        ...
        int i = 0;
        while (!stack.isEmpty()) {
            ch[i++] = stack.pop();//popeline aldıgı karekteri silerek alır//i++sayı arttırmayı içine aldık
        }
        System.out.println("Stack ile " + String.copyValueOf(ch));// köşeli parantez ve aradaki vilgülden kurtulmak içim
        System.out.println("Stack ile " + new String(ch));// köşeli parantez ve aradaki vilgülden kurtulmak içim
